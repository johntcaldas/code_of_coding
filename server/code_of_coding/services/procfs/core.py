...
from code_of_coding.services.procfs.exceptions import \
    DoesNotExist, UnknownProcess, NoParentProcess, NotDirectory, NotFile
from code_of_coding.services.procfs.utils import get_module
...
class File(BaseFile):
    ...
    def __init__(self, filepath=None):
        if not filepath:
            name = self.__module__[len('code_of_coding.services.procfs.proc.'):]
...
class ProcessFile(BaseFile):
    ...
    def __init__(self, id, filepath=None):
        if not filepath:
            name = self.__module__[len('code_of_coding.services.procfs.processes.'):]
...
class ProcDirectory(BaseDirectory):
    ...
    _base_module = 'code_of_coding.services.procfs.proc'
    _skip_path_parts = 2
...
class ProcessDirectory(BaseDirectory):
    ...
    _base_module = 'code_of_coding.services.procfs.processes'
    _skip_path_parts = 3
